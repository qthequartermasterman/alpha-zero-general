...
from .UnoLogicMultiplayer import UnoBoard as Board
from .DeckOfCards import size_of_deck
import numpy as np


class UnoGame(Game):
    def __init__(self, num_players=7, cards_dealt_per_player=7):
    ...
    def getNextState(self, board, player_list, action):
        # if player takes action on board, return next (board,player)
        # action must be a valid move
        b = self.board
        b.from_numpy(board, player_list)
        return b.execute_move(action, player_list[0])

    def getValidMoves(self, board, player_list, player):
        # return a fixed size binary vector
        b = self.board
        b.from_numpy(board, player_list)
        return b.get_valid_moves_numpy(player)

    def getGameEnded(self, board, player_list, player):
        # return 0 if not ended, 1 if input player won, -1 if input player lost
        # player = 1
        b = self.board
        b.from_numpy(board, player_list)
        if b.has_empty_hand(player_list[0]):
            return 1
        if b.is_game_over():
            return -1
        return 0

    def getCanonicalForm(self, board, player_list):
    ...
    def print_board_human_friendly(self, board, player_list):
        self.board.from_numpy(board, player_list)
