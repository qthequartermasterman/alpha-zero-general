...
class UnoBoard:
    ...
    def get_next_player(self, current_player, skip=False, reverse=False):
        ...
        direction = self.direction_of_play if not reverse else -self.direction_of_play
        return (current_player + direction * number_of_players_to_move) % len(self.players)

    def play_card(self, player: int, card_id:int):
        """"""
        card = master_list_of_cards[card_id]
        draw_number = card.draw_number
        skip = card.skip
        reverse = card.reverse
        self.players[player].play_card_by_id(card_id)
        if reverse:
    ...
    def get_board_numpy(self):
        ...
        return np.stack(list_of_np_arrays), player_list

    def from_numpy(self, x, player_list):
        self.deck.from_numpy(x[:1])
        self.discard.from_numpy(x[1:2], x[2:3])
        self.players.clear()
        hands_np = x[3:]
        self.players = [None]*len(player_list)  # Initialize an empty array so we can do a c-style loop.
        # Not the most pythonic way, but it works.
        for i in range(len(player_list)):
            player_id = player_list[i]
            hand_object = Hand(self.deck, self.discard)
            hand_object.from_numpy(hands_np[i])
            #self.players.append(hand_object)
            self.players[player_id] = hand_object
        if player_list[0] != 0:
            if player_list[1] > player_list[0]:
                self.direction_of_play = 1
            else:
                self.direction_of_play = -1
        else:
            if player_list[2] > player_list[1]:
                self.direction_of_play = 1
            else:
                self.direction_of_play = -1
    ...
    def execute_move(self, action_id, player):
        ...
        if action_id not in [-1, size_of_deck]:
            ...
            self.play_card(player, action_id)
            if self.discard.top_card().skip:
                skip = True
            ...
            self.curPlayer = self.get_next_player(self.curPlayer, skip=False, reverse=False)
        return self.get_board_numpy()
